...
exports.create = async (req, res) => {
  try {
    const { title, company_name, category, description, location } = req.body;
    await pool.query(
      'INSERT INTO jobs (title, company_name, category, description, location, created_by) VALUES (?,?,?,?,?,?)',
      [title, company_name, category, description, location, req.user.id]
    );
    res.json({ ok: true });
  } catch (e) {
    console.error(e);
...
exports.update = async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [title, company_name, category, description, location, req.params.id]
    );
    res.json({ ok: true });
  } catch (e) {
    console.error(e);
