...
export default function AdminProfileForm() { 
  ...
  return (
    <div className="card shadow-lg rounded-4 border-0 overflow-hidden">
      <div className="card-header text-white p-4" style={{ background: 'linear-gradient(135deg, #4548c9ff, #447bdaff)' }}>
        <div className="d-flex align-items-center">
          <img
            src={ me.avatar ? `http://localhost:5000${me.avatar}` : "https://via.placeholder.com/80" }
            alt="profile"
        ...
        <form onSubmit={submit}>
          ...
          <div className="row g-3">
            <div className="col-md-6">
              ...
              <input className="form-control shadow-sm" value={name} onChange={e => setName(e.target.value)} required />
            </div>
            <div className="col-md-6">
              <label className="form-label fw-semibold">Email</label>
              <input className="form-control shadow-sm" type="email" value={email} onChange={e => setEmail(e.target.value)} required />
            </div>
            <div className="col-md-6">
              <label className="form-label fw-semibold">Contact</label>
              <input className="form-control shadow-sm" value={contact} onChange={e => setContact(e.target.value)} placeholder="10-digit mobile" />
            </div>
            <div className="col-md-6">
              <label className="form-label fw-semibold">Education</label>
              <input className="form-control shadow-sm" value={education} onChange={e => setEducation(e.target.value)} />
            </div>
            <div className="col-3">
              <label className="form-label fw-semibold">Profile Photo</label>
              <input className="form-control shadow-sm" type="file" onChange={e => setAvatar(e.target.files[0])} accept="image/*" />
            </div>
          </div>

          <div className="mt-4 d-flex justify-content-end">
            <button className="btn btn-primary me-2 btn-sm" type="submit" disabled={saving}>
